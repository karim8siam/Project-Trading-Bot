"""
Refined High-Conviction Benchmark:
Applies the 4 Data-Driven Refinements from the 2nd 100-Trade Analysis:
1. ML Confidence Gate >= 0.72 (Filters out the 22% win-rate chop bucket)
2. Max 1 Concurrent Position (Eliminates 4-pair correlation dump)
3. Confluence Score >= 160 (Grade S / Grade A Order Blocks with Fib 61.8% Golden Ratio)
4. Dynamic Asymmetric 1:3 R:R Payoff with Breakeven at 1R and Trailing SL
"""
...
from merge_engine import combine_rule_and_ml_scores


def run_refined_high_conviction_benchmark(target_trades: int = 50, initial_balance: float = 5000.0) -> Dict[str, Any]:
    """
    Executes trades using the refined high-conviction ML settings.
    """
    print("=" * 85, flush=True)
    print("🎯 RUNNING REFINED HIGH-CONVICTION ML BENCHMARK", flush=True)
    print(f"Capital: ${initial_balance:,.2f} USDT | Pairs: {ALLOWED_SYMBOLS}", flush=True)
    print("Settings: ML Confidence >= 72% | Max 1 Position | Score >= 160 | 1:3 R:R", flush=True)
    print("=" * 85, flush=True)
    ...
    for sym in ALLOWED_SYMBOLS:
        market_dfs[sym] = fetcher.generate_realistic_ohlcv(sym, n_candles=5000, timeframe="15m")

    sim_index = 250
    while len(completed_trades) < target_trades and sim_index < 4900:
        # Check active position progression first
        if active_position:
            ...
            if exit_price:
                ...
                print(f"  [Trade {len(completed_trades)}] {active_position['trade_id']} ({sym} {direction}) -> {exit_reason} ({'+' if pnl_usd >= 0 else ''}${pnl_usd:.2f}) | Balance: ${current_balance:,.2f} USDT", flush=True)

                close_trade(active_position["trade_id"], exit_price=exit_price, exit_reason=exit_reason)
                active_position = None

        # Look for new high-conviction trade only if no active position
        if not active_position:
            for symbol in ALLOWED_SYMBOLS:
                ...
                # Rule 3: Asymmetric 1:3 R:R Target
                ...
                trade_counter += 1
                trade_id = f"REFINED-{trade_counter:03d}"
...
def print_refined_report(res: Dict[str, Any]):
    print("\n" + "=" * 90, flush=True)
    print("      🏆 REFINED HIGH-CONVICTION BENCHMARK RESULTS", flush=True)
    print("=" * 90, flush=True)
    ...
    summary_table = [
        ...
        ["Total High-Conviction Trades", f"{res['total_trades']}", "Win Rate", f"{res['win_rate_pct']}% ({res['wins']}W / {res['losses']}L)"],
        ...
        ["Rule 6 Breakeven Saves", f"{res['breakeven_saves']} Trades Saved", "ML Chop Traps Filtered", f"{res['traps_filtered_by_ml']} Traps Blocked 🔥"]
    ]
    print(tabulate(summary_table, tablefmt="fancy_grid"), flush=True)

    print("\n[RECENT 10 TRADES DETAIL LOG]", flush=True)
...
if __name__ == "__main__":
    refined_res = run_refined_high_conviction_benchmark(target_trades=30, initial_balance=5000.0)
    print_refined_report(refined_res)
