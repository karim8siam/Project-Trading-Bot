...
class BinanceFuturesExecutor:
    ...
    def check_and_execute_portfolio_unrealized_pnl_harvest(
        ...
        try:
            ...
            from config import BASKET_WIN_TARGET_PERCENT
            ...
            print(f"  [Basket Monitor] 📊 Open Positions: {len(live_active_positions)} | Unrealized PnL: ${total_unrealized_profit:+.4f} ({pnl_pct:+.2f}% / Win Target: +{BASKET_WIN_TARGET_PERCENT:.2f}% | SL: Individual Native Orders)")
            ...
            if total_unrealized_profit >= target_profit_threshold:
                ...
                print(f"  • Action: Executing MKT Close All across {len(live_active_positions)} positions to bank profit!")
                print("=" * 80)
